...
async function getApprentice(
  ...
  const format = req.query.get('format') === 'jsonld' ? 'jsonld' : negotiateFormat(req);
...
async function listApprentices(
  ...
  const pageSize = Math.min(
    config.maxPageSize,
    Math.max(1, parseInt(req.query.get('size') ?? String(config.defaultPageSize)))
  );
  const offset   = (page - 1) * pageSize;
  const socFilter = req.query.get('soc');

  ctx.log(`GET /id/apprentice (page: ${page}, size: ${pageSize}, soc: ${socFilter ?? 'all'})`);

  try {
    const db       = getDb();
    const where    = socFilter ? 'WHERE soc_code = ?' : '';
    const params   = socFilter ? [socFilter, pageSize, offset] : [pageSize, offset];
    ...
    const rows  = db.prepare(
      `SELECT apprentice_id, soc_code, official_title, starting_wage, state, fiscal_year
       ...
       LIMIT ? OFFSET ?`
    ).all(...params) as Array<Pick<ApprenticeRow, 'apprentice_id' | 'soc_code' | 'official_title' | 'starting_wage' | 'state' | 'fiscal_year'>>;

    const total = (db.prepare(
      `SELECT COUNT(*) as n FROM apprentices ${where}`
      ...
      return {
        ...
        jsonBody: {
          ...
          'hydra:member': rows.map(r => ({
            ...
            '@type': 'dol:Apprentice',
            'schema:occupationalCategory': { '@id': config.uris.occupation(r.soc_code) },
